 ...
 */
final class DefaultDrawerAnimationController implements DrawerAnimationController {
    ...
    DefaultDrawerAnimationController(DrawerAnimationController.Config config) {
        Objects.requireNonNull(config, "config");
        this.drawerSupplier = Objects.requireNonNull(config.drawerSupplier(), "drawerSupplier");
        this.controlSupplier = config.controlSupplier();
        this.edge = Objects.requireNonNull(config.edge(), "edge");
        this.durationMs = Math.max(1, config.durationMs());
        this.minFrameDelayMs = Math.max(1, config.minFrameDelayMs());
        this.maxFrameDelayMs = Math.max(this.minFrameDelayMs, config.maxFrameDelayMs());
        this.repaintPaddingPx = Math.max(0, config.repaintPaddingPx());
        this.onTargetStateChanged = config.onTargetStateChanged();
